...
function initWebmeji() {
  if (!window.SPAWNING || !Array.isArray(window.SPAWNING)) {
    console.error('webmeji: window.SPAWNING não foi encontrado');
    return;
  }
  ...
  Promise.all(configs.map(preloadImages))
    .then(() => {
      ...
      window.SPAWNING.forEach(({ id, config }) => {
        const spriteConfig = window[config];
        if (spriteConfig) {
          creatures.push(new Creature(id, spriteConfig));
        }
      });

      console.log('creatures created:', creatures.length);
    })
    .catch(error => {
...
if (document.readyState === 'loading') {
  window.addEventListener('DOMContentLoaded', initWebmeji, { once: true });
} else {
  initWebmeji();
}
